...
<script>
import { epicDIR, ewalletPath} from '../shared/config.js'
const fse = window.nodeFsExtra;
  ...
  methods: {
    ...
    remove(){
      ...
      if(['移除', 'remove'].indexOf(rcText) >= 0){
        ...
        let dest = path.join(ewalletPath, `.epic_${suffix}`)
        fse.moveSync(epicDIR, dest)
        this.removed = true
      }else{
    ...
    closeModal() {
      this.emitter.emit('closeWindowRemove');
      this.noConfirmed = false
    }
  }
}
